...
// numero di celle per un singolo spostamento
...
void move(int** portionMatrix, const int& old_i, const int& old_j, int new_i, int new_j)
{
...
void move_gas(int** portionMatrix, int i,  int j,  int portionSize) 
{
    ...
    // UP
    if (i - 3 >= 1 && portionMatrix[i-3][j] == EMPTY)
        possible_directions.push_back(UP);

    // DOWN
    if (i + 3 < portionSize && portionMatrix[i+3][j] == EMPTY)
        possible_directions.push_back(DOWN);
    
    // Left
    if (j - 3 >= 0 && portionMatrix[i][j - 3] == EMPTY)
        possible_directions.push_back(LEFT);

    // Right
    if (j + 3 < SIZE && portionMatrix[i][j + 3] == EMPTY)
    ...
    {
    case LEFT:
        move(portionMatrix, i, j, i, j - 3);
        return;
    case RIGHT:
        move(portionMatrix, i, j, i, j + 3);
        return;
    case UP:
        move(portionMatrix, i, j, i-3, j);
        return;
    case DOWN:
        move(portionMatrix, i, j, i+3, j);
        return;
...
int main(int argc, char** argv) 
{
        ...
        while (continue_automa== 1) 
        {
            ...
            for (int rank = 1; rank<num_procs; rank++) 
            {
                ...
                for (int i = 0; i < portionSize; ++i) 
                {
                    ...
                    MPI_Isend(map[i + rank_portion], 1, rowDataType, rank, OLD_MAP_SEND_RCV_TAG, MPI_COMM_WORLD, &request);
            ...
            for (int rank = 1; rank < num_procs; rank++) 
            {
                for (int i = 0; i < portionSize; i++) 
                {
                    MPI_Recv(map[i + rank_portion], 1, rowDataType, rank, NEW_MAP_SEND_RECV_TAG, MPI_COMM_WORLD, &status);
                }
                rank_portion += portionSize; // per il prossimo processo
            }
    ...
    else 
    {
        ...
        while (continue_automa== 1) 
        {
            ...
            MPI_Irecv(portionMatrix[0], 1, rowDataType, top_des, BOTTOM_BORDER_SEND_RECV, MPI_COMM_WORLD, &topRecvRequest);

            // ricevo il mio bordo inferiore ovvero la prima riga della sottomatrice del processo successivo
            MPI_Irecv(portionMatrix[portionSize + 1], 1, rowDataType, bottom_des, TOP_BORDER_SEND_RECV, MPI_COMM_WORLD, &bottomRecvRequest);

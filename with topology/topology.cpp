...
int main(int argc, char** argv) 
{
    ...
    MPI_Comm newComm;
    MPI_Comm cartComm;
    MPI_Group generale, newGroup; 
         
//********************************************************************************************************************************************************************************************************************************************************************************************************////
    //creo un gruppo generale con tutti i processi a partire da comm_world
    MPI_Comm_group(MPI_COMM_WORLD, &generale);
    //sto creando un gruppo di processi eliminando il processo 0, dentro questo gruppo avrò da 1 a numprocs-1
    int esclude[1]={0};
    //std::cout<<esclude[0];
    //1 sarebbe la grandezza dell'array dei processi da escludere
    MPI_Group_excl(generale, 1, esclude, &newGroup);

    
    int my_rank;
    MPI_Group_rank(newGroup, &my_rank);
    //std::cout<<"sono il rank " <<my_rank<<"\n";

    
    //creo un comunicatore sul nuovo gruppo prendendo i processi dal MPI_COMM_WORLD e mettendoli
    // dentro newComm, ottengo così un nuovo comunicatore con i processi da 1 a numprocs-1
    MPI_Comm_create(MPI_COMM_WORLD, newGroup, &newComm);

    //printf("dopo del comm_create\n");
    ...
    if (rank == 0) 
    {
        ...
        // creo e inizializzo il display e l'utilizzo della keyboard
        ALLEGRO_DISPLAY* display;
        ALLEGRO_KEYBOARD_STATE key_state;
        al_init();
        const int DISPLAYHEIGHT = SIZE * DIM;
        const int DISPLAYWIDTH = SIZE * DIM;
        display = al_create_display(DISPLAYWIDTH, DISPLAYHEIGHT);
        al_init_primitives_addon();
        al_install_keyboard();
        
        /*if (!al_install_keyboard()) {
            printf("Errori con la tastiera. Termine programma...\n");
            MPI_Finalize();
            return -1;
        }*/

        while (continue_automa== 1) 
        {
 ...
 //********************************************************************************************************************************************************************************************************************************************************************************************************////
        //mi creo la topologia virtuale, che avrà grandezza numprocs-1 perchè dentro non c'è il processo 0
        int dim[1]={num_procs-1};
        //voglio che orizzontalemte sia toroidale ovvero il processo 1 a sinistra deve avere numprocs-1
        // mentre numprocs-2 a destra deve avere 1 
        int period[1]={1};

        //creo la topologia a partire dal comunicatore nuovo, e la imposto sul comunicatore cartComm
        //1 sarebbe il numero di dimensioni, in questo caso 1 dimensione sola, una sorta di riga di processi
        MPI_Cart_create(newComm, 1, dim, period, 0, &cartComm);

        while (continue_automa== 1) 
        {
            int bottom_des=0; // destinatario bordo inferiore
            int top_des=0; // destinatario bordo superiore
            ...
            //mi prendo il vicino destro e sinistro per ogni processo e lo metto in topdes e bottomdes
            //0 sarebbe la riga su cui mi sposto e 1 sarebbe  destra e sinistra di 1 elemento
            MPI_Cart_shift(cartComm, 0,1, &top_des, &bottom_des);

            std::cout<<"sono il rank " <<myrank<<" top= "<<top_des<<" bottom= "<<bottom_des<<"\n";
            ...
            MPI_Isend(portionMatrix[1], 1, rowDataType, top_des, TOP_BORDER_SEND_RECV, cartComm, &topSendRequest);

            // invio la mia ultima riga, che sarà il bordo superiore del prossimo processo
            MPI_Isend(portionMatrix[portionSize], 1, rowDataType, bottom_des, BOTTOM_BORDER_SEND_RECV, cartComm, &bottomSendRequest);

            // ricevo il mio bordo superiore ovvero la l'ultima riga della sottomatrice del processo precedente
            MPI_Irecv(portionMatrix[0], 1, rowDataType, top_des, BOTTOM_BORDER_SEND_RECV, cartComm, &topSendRequest);

            // ricevo il mio bordo inferiore ovvero la prima riga della sottomatrice del processo successivo
            MPI_Irecv(portionMatrix[portionSize + 1], 1, rowDataType, bottom_des, TOP_BORDER_SEND_RECV, cartComm, &bottomSendRequest);
          ...
          // invio la mia prima riga, che sarà il bordo inferiore del processo precedente
            MPI_Isend(portionMatrix[0], 1, rowDataType, top_des, TOP_BORDER_SEND_RECV, cartComm, &topSendRequest);

            // invio la mia ultima riga, che sarà il bordo superiore del prossimo processo
            MPI_Isend(portionMatrix[portionSize+1], 1, rowDataType, bottom_des, BOTTOM_BORDER_SEND_RECV, cartComm, &bottomSendRequest);

            // ricevo il mio bordo superiore ovvero la l'ultima riga della sottomatrice del processo precedente
            MPI_Irecv(sup, 1, rowDataType, top_des, BOTTOM_BORDER_SEND_RECV, cartComm, &topSendRequest);

            // ricevo il mio bordo inferiore ovvero la prima riga della sottomatrice del processo successivo
            MPI_Irecv(inf, 1, rowDataType, bottom_des, TOP_BORDER_SEND_RECV, cartComm, &bottomSendRequest);
